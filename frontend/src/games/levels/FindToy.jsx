...
function FindToy({ onComplete }) {
  ...
  const finishRound = () => {
    ...
    // Очки: за правильный выбор +1, за ошибку -1, бонус за все правильные без ошибок +5
    let roundScore = correctSelected.length - wrongSelected.length
    if (wrongSelected.length === 0 && unselectedCorrect.length === 0) {
      roundScore += 5 // Бонус за идеальный раунд
    }
  ...
  if (!gameStarted) {
    return (
      <div className="find-toy">
        ...
        <p style={{color: '#ffaa00', marginTop: '1rem'}}>💰 За правильный выбор: <strong>+1 балл</strong></p>
        <p style={{color: '#ff4444', marginTop: '0.5rem'}}>❌ За ошибку: <strong>-1 балл</strong></p>
        <p style={{color: '#44ff44', marginTop: '0.5rem'}}>⭐ Бонус за идеальный раунд: <strong>+5 баллов</strong></p>
